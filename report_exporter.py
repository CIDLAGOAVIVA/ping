...
class ReportExporter:
    ...
    def export_to_csv(metrics: Dict[str, Any], filename: Optional[str] = None) -> str:
        ...
        if emerging.get('total_topics', 0) > 0:
            ...
            writer.writerow(['=== HASHTAGS EM DESTAQUE ==='])
            writer.writerow(['Hashtag', 'Menções'])
            for hashtag in emerging.get('top_hashtags', [])[:10]:
                writer.writerow([
                    f"#{hashtag.get('tag', '')}",
                    hashtag.get('count', 0)
                ])
    ...
    def export_to_pdf(metrics: Dict[str, Any], filename: Optional[str] = None) -> bytes:
        ...
        if emerging.get('total_topics', 0) > 0:
            ...
            # Hashtags
            elements.append(Paragraph("HASHTAGS EM DESTAQUE", heading_style))
            
            hashtags_data = [['#', 'Hashtag', 'Menções']]
            for i, hashtag in enumerate(emerging.get('top_hashtags', [])[:10], 1):
            ...
            hashtags_table = Table(hashtags_data, colWidths=[0.5*inch, 2.5*inch, 1.5*inch])
